    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          /* Navy background with gold bottom border */
        ...
        >
          <div className="flex items-center gap-3 px-4 py-2 max-w-screen-2xl mx-auto">
                ...
                {[...TENDERS, ...TENDERS].map((t, i) => (
                  <div key={i} className="inline-flex items-center gap-2.5 text-sm">
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full shrink-0 ${
                      t.status === 'New' ? 'badge-gold' : 'bg-blue-800 text-blue-200'
                    }`}>{t.status}</span>
            ...
            <button onClick={() => setVisible(false)} className="p-1 text-blue-400 hover:text-white transition-colors shrink-0" aria-label="Close tender banner">
              <X className="h-4 w-4" />
            </button>
          </div>
        </motion.div>
