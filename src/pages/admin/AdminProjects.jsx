...
import { Plus, Edit2, Trash2, X, CheckCircle, AlertTriangle, Briefcase } from 'lucide-react';
...
const AdminProjects = () => {
  ...
  const [create, {loading:creating}] = useMutation(projectsApi.create);
  const [update, {loading:updating}] = useMutation(projectsApi.update);
  const [remove]                      = useMutation(projectsApi.delete);
        ...
        {(showForm || editing) && (
          <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}}
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-white text-lg">{editing ? 'Edit' : 'Add'} Project</h3>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors group">
                      <td className="px-4 py-3">
                        {(item.imageUrl || item.image)
                          ...
                          : <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center"><Briefcase className="h-5 w-5 text-gray-300"/></div>
                        }
                      </td>
                      <td className="px-4 py-3">
        ...
        {items.length===0 && !loading && (
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-sm mt-1">Click "Add Project" to create the first entry</p>
          </div>
        )}
      </div>
    </div>
