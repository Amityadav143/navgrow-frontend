...
// ESM-compatible __dirname (package.json has "type":"module" so CJS globals are unavailable)
...
if (isDev) {
	...
	} catch { /* visual editor plugins not available — skipping */ }
}
...
export default defineConfig({
  define: {
    // Embed env vars at build time (needed for Docker multi-stage builds)
    'import.meta.env.VITE_API_BASE_URL':    JSON.stringify(process.env.VITE_API_BASE_URL || 'http://localhost:8080/api'),
    'import.meta.env.VITE_RAZORPAY_KEY_ID': JSON.stringify(process.env.VITE_RAZORPAY_KEY_ID || 'rzp_test_1DP5mmOlF5G5ag'),
  },
	customLogger: logger,
	plugins: [
